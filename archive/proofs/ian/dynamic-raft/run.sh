...
run_3n() {
  echo
  echo "3n"
  #timeout --foreground 30s java -cp ~/bin/tla2tools.jar tlc2.TLC -config MongoLoglessDynamicRaftProofs.cfg MongoLoglessDynamicRaftProofs.tla -workers 1 | tee -a $log3
  timeout --foreground 30s java -cp ~/bin/tla2tools.jar tlc2.TLC -config MongoLoglessDynamicRaftProofs_3n.cfg MongoLoglessDynamicRaftProofs.tla -workers 1 | tee -a $log3
}

run_5n() {
  echo
  echo "5n"
  timeout --foreground 30s java -cp ~/bin/tla2tools.jar tlc2.TLC -config MongoLoglessDynamicRaftProofs_5n.cfg MongoLoglessDynamicRaftProofs.tla -workers 1 | tee -a $log5
}
