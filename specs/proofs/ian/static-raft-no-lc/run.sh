...
run_3n() {
  echo
  echo "3n"
  timeout --foreground 30s java -cp ~/bin/tla2tools.jar tlc2.TLC -config MongoStaticRaftProofs_3n.cfg MongoStaticRaftProofs.tla -workers 1 | tee -a $log3
  #timeout --foreground 5m java -cp ~/bin/tla2tools.jar tlc2.TLC -config MongoStaticRaftProofs_3n.cfg MongoStaticRaftProofs.tla -workers 8 | tee -a $log3
}

run_5n() {
  echo
  echo "5n"
  timeout --foreground 30s java -cp ~/bin/tla2tools.jar tlc2.TLC -config MongoStaticRaftProofs_5n.cfg MongoStaticRaftProofs.tla -workers 1 | tee -a $log5
  #timeout --foreground 5m java -cp ~/bin/tla2tools.jar tlc2.TLC -config MongoStaticRaftProofs_5n.cfg MongoStaticRaftProofs.tla -workers 8 | tee -a $log5
}
