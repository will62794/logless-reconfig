...
PROOF
    ...
    <1>4. \A i \in DOMAIN log[s] : P(i) => P(i-1)
        ...
        <2>2. \A i \in DOMAIN log[s] : (log[s][i] = log[s][upper]) => log[s][i-1] >= log[s][upper] BY <1>1, <2>1 DEF Ind, TypeOK
    ...
    <1>9. 1 \in DOMAIN log[s] /\ upper \in DOMAIN log[s] BY DEF Ind, TypeOK
...
PROOF
    ...
    <1>6. configTerm[n] > currentTerm[p]
        ...
        <2>2. configTerm[n] >= configTerm[p] BY <1>2 DEF CSM!NewerConfig, CV, Ind, TypeOK
        ...
        <2>4. configTerm[n] > configTerm[p] BY <1>n, <1>2, <2>2, <2>3 DEF CSM!NewerConfig, CV, Ind, TypeOK
